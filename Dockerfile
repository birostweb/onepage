# ============================================================
#  One Page Challenge — build des 5 mini-sites + service statique
...
# Build des 4 mini-sites Vue+Vite en base relative (pour fonctionner sous /<nom>/),
# puis assemblage de la galerie dans /site.
RUN set -e; \
    for d in Sonos brf flyhigh tract; do \
      echo "==> build $d"; \
      (cd "$d" && npm ci && npm run build -- --base=./); \
    done; \
    mkdir -p /site; \
    cp index.html /site/index.html; \
    cp -r Sonos/dist   /site/Sonos; \
